import { existsSync } from 'node:fs';
import { join } from 'node:path';
import { resolveStorePath, readJSONL } from '../repo-store.js';
import { redactContent } from '../redaction-engine.js';
import { acquireLock, releaseLock, LockTimeoutError } from '../lock.js';
import {
  ensureClone,
  hasRemote,
  fetch,
  pullRebase,
  stashSave,
  stashPop,
  RebaseConflictError,
  getHead,
  stageFile,
  commit,
  push
} from '../git.js';
import { mergePendingToStore } from '../merge.js';
import { rebuildIndex, updateIndex, getIndexStatus } from '../index-store.js';
import { compactMemories } from '../compact-engine.js';

/**
 * mem-sync flush 命令：提交并推送合并后的记忆记录。
 *
 * 8 步序列：
 * 1. 确保 .mem-sync 仓库存在（clone 或 init）
 * 2. 获取排他文件锁
 * 3. Git 同步：fetch + pull/rebase（stash 保护）
 * 4. 确定性合并：pending/ → memories.jsonl
 * 5. 如果有合并记录：stage → commit → push（非阻塞）
 * 6. 索引更新/重建（基于 HEAD 变化）
 * 7. 释放锁（finally 保证）
 * 8. JSON 结果写入 stdout
 *
 * 输出：JSON 结果写入 stdout；诊断/进度/警告写入 stderr。
 *
 * @param {string[]} args - CLI 参数（支持 --remote <url>）
 */
export async function flushCommand(args) {
  const memSyncHome = process.env.MEM_SYNC_HOME ?? '.mem-sync';
  const lockPath = join(memSyncHome, 'repo.lock');
  const pendingDir = join(memSyncHome, 'pending');
  const storePath = resolveStorePath(memSyncHome);
  const cacheDir = join(memSyncHome, '.cache');

  // 解析 --remote 参数
  const remoteIdx = args.indexOf('--remote');
  const remoteUrl =
    remoteIdx !== -1 && remoteIdx + 1 < args.length
      ? args[remoteIdx + 1]
      : null;
  // 解析 --compact 参数
  const compactFlag = args.includes('--compact');
  // 解析 --skip-redaction 参数
  const skipRedaction = args.includes('--skip-redaction');

  // 结果对象
  const result = {
    git: { skipped: false, pulled: 0, conflicts: 0 },
    merge: { pending: 0, merged: 0, total: 0 },
    compact: null,
    commit: { made: false, hash: null },
    push: { attempted: false, success: false },
    index: { rebuilt: false, records: 0 }
  };

  let lockAcquired = false;

  try {
    // ── Step 1: Ensure repository exists ──────────────────────────
    if (!existsSync(join(memSyncHome, '.git'))) {
      ensureClone(remoteUrl, memSyncHome);
      console.error('[mem-sync:flush] ensure:repository ready');
    }

    // ── Step 2: Acquire repository lock ──────────────────────────
    console.error('[mem-sync:flush] lock:acquiring');
    try {
      await acquireLock(lockPath);
      lockAcquired = true;
      console.error('[mem-sync:flush] lock:acquired');
    } catch (err) {
      if (err instanceof LockTimeoutError) {
        console.error(`mem-sync: lock timeout — another process may be using the repository`);
        process.exitCode = 1;
        return; // 不输出 JSON
      }
      throw err;
    }

    // ── Step 3: Git sync ─────────────────────────────────────────
    if (hasRemote(memSyncHome)) {
      console.error('[mem-sync:flush] git:syncing');

      // Fetch
      let fetchCount;
      try {
        fetchCount = fetch(memSyncHome);
        console.error(`[mem-sync:flush] git:fetched ${fetchCount} new commits`);
      } catch {
        console.error('[mem-sync:flush] git:fetch failed, continuing with local state');
        fetchCount = 0;
      }

      // Pull/rebase with stash protection
      const stashed = stashSave(memSyncHome);
      if (stashed) {
        console.error('[mem-sync:flush] git:stashed local changes');
      }

      try {
        const pulled = pullRebase(memSyncHome);
        result.git.pulled = pulled;
        console.error(`[mem-sync:flush] git:pulled ${pulled} commits`);
      } catch (err) {
        if (err instanceof RebaseConflictError) {
          result.git.conflicts = 1;
          console.error('mem-sync: rebase conflict — manual resolution required');
          process.exitCode = 1;
          return;
        }
        throw err;
      } finally {
        if (stashed) {
          try {
            stashPop(memSyncHome);
            console.error('[mem-sync:flush] git:stash popped');
          } catch (popErr) {
            console.error(`[mem-sync:flush] git:stash pop failed: ${popErr.message}`);
          }
        }
      }
    } else {
      result.git.skipped = true;
      console.error('[mem-sync:flush] git:skipped (no remote configured)');
    }

    // ── Step 4: Deterministic merge ──────────────────────────────
    console.error('[mem-sync:flush] merge:starting');
    const mergeResult = mergePendingToStore(pendingDir, storePath);
    result.merge = mergeResult;
    console.error(
      `[mem-sync:flush] merge:complete pending=${mergeResult.pending} merged=${mergeResult.merged} total=${mergeResult.total}`
    );
    // ── Step 4.5: Compact (opt-in) ──────────────────────────────
    if (compactFlag) {
      console.error('[mem-sync:flush] compact:starting');
      try {
        const compactResult = await compactMemories({
          storePath,
          olderThanDays: 30,
          dryRun: false
        });
        if (compactResult.candidates > 0) {
          result.compact = compactResult;
          console.error(
            `[mem-sync:flush] compact:done candidates=${compactResult.candidates} duplicates=${compactResult.duplicates} removed=${compactResult.removed} kept=${compactResult.kept}`
          );
        } else {
          result.compact = compactResult;
          console.error('[mem-sync:flush] compact:no candidates');
        }
      } catch (err) {
        console.error(`[mem-sync:flush] compact:warning — ${err.message}`);
        // Compact is best-effort; flush should still succeed
      }
    }

    // ── Step 4.8: Redaction gate ──────────────────────────────
    if (!skipRedaction && mergeResult.merged > 0) {
      console.error('[mem-sync:flush] redaction:scanning');
      const storeRecords = await readJSONL(storePath);
      const blockedRecords = [];
      for (const record of storeRecords) {
        if (!record.content) continue;
        const redactResult = redactContent(record.content);
        if (redactResult.blocked) {
          blockedRecords.push({ id: record.id, rules: redactResult.matches.map(m => m.rule) });
        }
      }
      if (blockedRecords.length > 0) {
        for (const br of blockedRecords) {
          console.error(`[mem-sync:redact] blocked record ${br.id}: ${br.rules.join(', ')}`);
        }
        console.error(`[mem-sync:flush] redaction:blocked ${blockedRecords.length} records, commit aborted`);
        result.redaction = { blocked: true, count: blockedRecords.length, records: blockedRecords };
        process.exitCode = 1;
        return;
      }
      console.error('[mem-sync:flush] redaction:clean');
    }

    // ── Step 5: Commit and push ──────────────────────────────────
    if (mergeResult.merged > 0) {
      console.error('[mem-sync:flush] commit:staging');
      stageFile(memSyncHome, 'memories.jsonl');

      const commitMessage = `mem-sync: merge ${mergeResult.merged} pending records (total: ${mergeResult.total})`;
      console.error(`[mem-sync:flush] commit:creating "${commitMessage}"`);
      try {
        const commitHash = commit(memSyncHome, commitMessage);
        result.commit = { made: true, hash: commitHash };
        console.error(`[mem-sync:flush] commit:created ${commitHash}`);
      } catch (err) {
        console.error(`[mem-sync:flush] commit:failed ${err.message}`);
        // 提交失败是致命的，但先输出结果再退出
        result.commit = { made: false, hash: null };
        process.exitCode = 1;
        return;
      }

      // Push (non-blocking)
      if (hasRemote(memSyncHome)) {
        result.push.attempted = true;
        console.error('[mem-sync:flush] push:attempting');
        try {
          const pushOk = push(memSyncHome);
          result.push.success = pushOk;
          if (pushOk) {
            console.error('[mem-sync:flush] push:success');
          } else {
            console.error('[mem-sync:flush] push:warning — push failed, commit is local only');
          }
        } catch {
          result.push.success = false;
          console.error('[mem-sync:flush] push:warning — push failed, commit is local only');
        }
      }
    } else {
      console.error('[mem-sync:flush] commit:skipped (no records merged)');
    }

    // ── Step 6: Index update/rebuild ─────────────────────────────
    console.error('[mem-sync:flush] index:checking');
    try {
      const currentHead = getHead(memSyncHome);
      const indexStatus = getIndexStatus(cacheDir);

      if (currentHead !== indexStatus.repoHead || !indexStatus.exists) {
        // HEAD 已更改或索引不存在 → 全量重建
        console.error('[mem-sync:flush] index:rebuilding');
        const rebuildResult = rebuildIndex(memSyncHome, cacheDir, {
          logger: (msg) => console.error(msg)
        });
        result.index = {
          rebuilt: true,
          records: rebuildResult.recordCount
        };
        console.error(`[mem-sync:flush] index:rebuilt ${rebuildResult.recordCount} records`);
      } else {
        // HEAD 未更改 → 增量更新
        console.error('[mem-sync:flush] index:updating');
        const updateResult = updateIndex(memSyncHome, cacheDir, {
          logger: (msg) => console.error(msg)
        });
        result.index = {
          rebuilt: updateResult.rebuilt ?? false,
          records: updateResult.recordCount ?? indexStatus.recordCount
        };
        if (updateResult.skipped) {
          console.error('[mem-sync:flush] index:uptodate (no changes)');
        } else {
          console.error(`[mem-sync:flush] index:updated ${result.index.records} records`);
        }
      }
    } catch (err) {
      console.error(`[mem-sync:flush] index:warning — ${err.message}`);
      // 非致命：索引过时但仍可操作
      result.index = { rebuilt: false, records: 0 };
    }

    // ── Step 7: Output result ────────────────────────────────────
    console.log(JSON.stringify(result));
  } catch (err) {
    // 致命错误
    console.error(`mem-sync: ${err.message}`);
    process.exitCode = 1;
  } finally {
    // ── Guaranteed lock release ──────────────────────────────────
    if (lockAcquired) {
      releaseLock(lockPath);
      console.error('[mem-sync:flush] lock:released');
    }
  }
}
